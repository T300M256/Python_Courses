"""
Email message handling module: contains logic to store and retreive
email message using a MySQL relation database.
"""
...
def store(msg):
    """
    Stores an email message, if necessary, returning its primary.
    """
...
def msg_by_id(i):
    """
    Retrun the (presumably singleton) message whose primary key is given
    or raise KeyError if no such message exists.
    """
    curs.execute("SELECT msgID, msgText FROM message WHERE msgID=%s", (i, ))
    result = curs.fetchone()
    if not result:
        raise KeyError("Id {0} not found in store".format(i))
    i, text = result
...
def msgs(mindate=None, maxdate=None, namesearch=None, addsearch=None):
    ...
    result = []
    for i, text in curs.fetchall():
        result.append((i, message_from_string(text)))
    return result