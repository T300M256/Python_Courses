...
class DBTest(unittest.TestCase):
    ...
    def test_retrieve_data_row_objects(self):
        ...
        self.assertEqual(observed_rows, expected_rows)   # compare observed with expected.
    ...
    def test_condition(self):
        ...
        cols = 'id name family weight'
        D = build_row(table, cols)
        sql = 'SELECT * FROM animal WHERE name="Gerald";'
        cursor.execute(sql)
        expected_rows = set( )
        for row in cursor.fetchall():
            expected_rows.add(repr(D(row)))
        ...
        d = D([100, "Joe", "Python", 10])  # making up data
        observed_rows = set( )
        for row in d.retrieve(cursor, condition='name="Gerald"'):
            observed_rows.add(repr(row))
