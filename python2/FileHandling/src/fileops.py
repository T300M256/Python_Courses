...
def write_list(fn, lst):
    ...
    a seperate line. Overwrites the file if it already exists.
    """
    f = open(fn, "w")
    for item in lst:
        f.write("%s\n" % item)
...
def read_list(fn):
    """Reads a list from a file with out using readline.
    Uses standard line endings ("\n") to delimit list items.
    """
    f = open(fn, "r")
    s = f.read()
    # if the last character in the file is a newline, delte it
    if s[-1] == "\n":
        s = s[:-1]
    l = s.split("\n")
    f.close()
